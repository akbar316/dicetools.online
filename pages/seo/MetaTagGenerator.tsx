...
const SerpPreview: React.FC<{ title: string; description: string; url: string }> = ({ title, description, url }) => (
    <div className="p-4 bg-brand-bg rounded-lg">
        <p className="text-sm text-brand-text-secondary truncate">{url || 'https://www.example.com'}</p>
        <h3 className="text-blue-500 text-xl truncate hover:underline">{title || 'Meta Title Appears Here'}</h3>
        <p className="text-sm text-brand-text-secondary">{description || 'This is where your meta description will be shown. Keep it concise and compelling to attract clicks from search results.'}</p>
    </div>
);
...
const MetaTagGenerator: React.FC = () => {
    ...
    const handleOptimize = async () => {
        if (!title.trim() && !description.trim()) {
            setError('Please provide a title or description to optimize.');
            return;
        }
        setLoading(true);
        setError('');
        try {
            ...
            const prompt = `Rewrite the following title and meta description to be more compelling and SEO-friendly for a website offering free online tools.\n\nTitle: "${title}"\nDescription: "${description}"`;
            ...
            setError(`An error occurred during optimization: ${e.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <ToolPageLayout
            ...
            description="Create optimized title and description tags, or let AI improve them for you."
        >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-4">
                    ...
                    <div>
                     ...
                     <button onClick={handleOptimize} disabled={loading} className="w-full bg-brand-primary text-white py-2 rounded-md hover:bg-brand-primary-hover disabled:bg-gray-500">
                        {loading ? <AiLoadingSpinner /> : 'Optimize with AI'}
                    </button>
